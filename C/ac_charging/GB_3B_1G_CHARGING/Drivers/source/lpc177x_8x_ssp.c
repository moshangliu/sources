/**********************************************************************
* $Id: lpc177x_8x_ssp.c 47021 2011-09-27 16:58:17Z anderslu $		lpc177x_8x_ssp.c			2011-06-02
*//**
...
** 备    注: 无
************************************************************************/
uint8_t SPIx_ReadWriteByte ( LPC_SSP_TypeDef *SSPx, uint8_t TxData )
{
    uint8_t temp;
    SSPx->DR = TxData;							// 发送该数据用以产生时钟,数据本身没有用处

    while ( 0 == SSP_GetStatus ( SSPx, SSP_STAT_TXFIFO_EMPTY ) );			// 等待SPIF置位，即等待数据发送完毕

    temp = SSPx->SR;								// 清除SPIF标志
    temp = temp;
    return ( uint8_t ) ( SSPx->DR );					// 返回接收到的数据
}
